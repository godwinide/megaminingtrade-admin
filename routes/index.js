...
router.post("/edit-user/:id", ensureAuthenticated, async (req,res) => {
    try{
        const {id} = req.params;
        const {balance, balance_bitcoin, total_deposit, total_deposit_bitcoin, pin, referral, referral_bitcoin, total_profit, total_profit_bitcoin, plans} = req.body;
        const customer = await User.findOne({_id:id}).limit(10)
        if(
            !balance || !balance_bitcoin 
            || !total_profit || !total_profit_bitcoin 
            || !total_deposit || !total_deposit_bitcoin
            || !pin || !plans
            || !referral || !referral_bitcoin
            ){
